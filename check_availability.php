...
$resultSeater ="SELECT seater FROM rooms WHERE room_no=?";
$stmtS = $mysqli->prepare($resultSeater);
$stmtS->bind_param('i',$roomno);
$stmtS->execute();
$stmtS->bind_result($countS);
$stmtS->fetch();
$stmtS->close();
...
if($count>0){
	if($count>$countS){
		echo "<span style='color:red'> Seats already full Room can't be allocated.</span>";
		echo '
		<script>
